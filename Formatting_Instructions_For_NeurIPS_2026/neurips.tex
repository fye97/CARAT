...
\paragraph{Datasets, models, and heterogeneity.}
We evaluate CARAT on CIFAR-10 with VGG19~\cite{simonyan2015verydeep} and on CIFAR-100 with ResNet18~\cite{he2016deep}. To study robustness under client heterogeneity, we partition client data with a Dirichlet distribution $\bm{p}\sim \mathrm{Dir}_N(\alpha)$ and report non-IID settings with $\alpha \in \{0.1, 0.5, 1\}$. The main-text tables and figures focus on the completed untargeted-poisoning slice of this benchmark; broader benchmark scope and implementation details are deferred to Appendix~\ref{sec:appendix-experiments}.

\paragraph{Baselines, attacks, and default training setting.}
Unless otherwise stated, we compare CARAT against FLTrust~\cite{cao2021fltrust}, FLDetector~\cite{zhang2022fldetector}, NormClipping, and MultiKrum~\cite{blanchard2017krum}, and we include a no-attack Mean reference where useful. The current main-text robustness results use four untargeted model-poisoning attacks: ALIE~\cite{baruch2019alie}, Min-Max~\cite{shejwalkar2021manipulating}, Min-Sum~\cite{shejwalkar2021manipulating}, and FangAttack~\cite{fang2020local}. The shared training configuration is summarized in Table~\ref{tab:hyperparams}; CARAT-specific defaults and the full repository benchmark matrix are recorded in Appendix~\ref{sec:appendix-experiments}.
\begin{table*}[!htbp]
\centering
\caption{Default training configuration for the main-text experiments.}
...
\subsection{Untargeted Poisoning Results}
  \begin{table*}[!htbp]
      \definecolor{mygray}{gray}{.90}
      \renewcommand{\arraystretch}{1.0}
      \setlength{\tabcolsep}{4pt}
      \caption{Final test loss under four untargeted model-poisoning attacks with $10\%$
  malicious clients on two dataset-model pairs under non-IID settings.}
...
\resizebox{\textwidth}{!}{%
  ...
  \pm$0.0001} & \textbf{0.0089$\pm$0.0002} & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0093$\pm$0.0002} &
  ...
  \pm$0.0006 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0106$\pm$0.0003} &
  ...
  \pm$0.0034 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0198$\pm$0.0016} &
  ...
  \textbf{0.0275$\pm$0.0005} & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}0.0276$\pm$0.0004 &
  ...
  \pm$0.0004} & \textbf{0.0283$\pm$0.0004} & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0284$\pm$0.0004} &
  ...
  \textbf{0.0323$\pm$0.0005} & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0322$\pm$0.0004} &
  ...
  \end{table*}
Table~\ref{tablossmergedcifar10cifar100} reports the completed untargeted-poisoning results with $10\%$ malicious clients. Across the reported non-IID settings, \texttt{CARAT} attains the best average rank and remains close to the no-attack Mean reference. The clearest gap appears on CIFAR-100/ResNet18 under FangAttack with $\alpha=0.1$, where \texttt{CARAT} reaches $0.0323$ test loss versus $0.0504$ for MultiKrum and $0.0450$ for FLTrust.
...
\begin{figure*}[!htbp]
 ...
 %  clip,scale=0.31]{figures/iid_CIFAR100_resnet18_FedAvg_adv0.2_alphaNA_clients20_lr0.05_cfgFedAvg_CIFAR100_config_epochs200.pdf}}
	% \quad
	\subfloat[Non-IID data distribution, $\alpha=1$]{\safeincludegraphics[trim=0 0 0 2.5cm,
  clip,scale=0.31]{figures/non-iid_CIFAR100_resnet18_FedAvg_adv0.2_alpha1_clients20_lr0.05_cfgFedAvg_CIFAR100_config_epochs200.pdf}}
	\quad
	\subfloat[Non-IID data distribution, $\alpha=0.5$]{\safeincludegraphics[trim=0 0 0 2.5cm,
  clip,scale=0.31]{figures/non-iid_CIFAR100_resnet18_FedAvg_adv0.2_alpha0.5_clients20_lr0.05_cfgFedAvg_CIFAR100_config_epochs200.pdf}}
    \quad
	\subfloat[Non-IID data distribution, $\alpha=0.1$]{\safeincludegraphics[trim=0 0 0 2.5cm,
  clip,scale=0.31]{figures/non-iid_CIFAR100_resnet18_FedAvg_adv0.2_alpha0.1_clients20_lr0.05_cfgFedAvg_CIFAR100_config_epochs200.pdf}}
    \caption{Test-accuracy trajectories on CIFAR-100/ResNet18 under untargeted poisoning with $20$ clients and $4$ attackers.}
...
Fig.~\ref{figaccuracycifar100} shows the same pattern on CIFAR-100/ResNet18: \texttt{CARAT} tracks the no-attack reference more closely than the competing defenses, while FLTrust and MultiKrum show larger degradation and instability. The gap widens as heterogeneity increases.

\paragraph{Impact of Attacker Ratio.}
We next increase the attacker ratio from the default $10\%$ to $20\%$ and $30\%$, keeping the remaining settings fixed. Fig.~\ref{figaccuracymoreattackerscifar100} shows that all methods degrade as the adversarial fraction grows, but the degradation for \texttt{CARAT} is consistently milder across the reported $\alpha$ values, whereas FLTrust and MultiKrum become unstable in part of this regime.
...
\begin{figure*}[htbp]
	...
	% \quad
	\subfloat[Dataset distribution is non-IID with $\alpha=1$.]{\safeincludegraphics[trim=0 0 0 2.5cm,
  clip,scale=0.31]{figures/adv_sensitivity_non-iid_CIFAR100_resnet18_FedAvg_alpha1_clients20_lr0.05_cfgFedAvg_CIFAR100_config_advset0.1-0.2-0.3_epochs200.pdf}}
	\quad
	\subfloat[Dataset distribution is non-IID with $\alpha=0.5$.]{\safeincludegraphics[trim=0 0 0 2.5cm,
  ...
  \quad
	\subfloat[Dataset distribution is non-IID with $\alpha=0.1$.]{\safeincludegraphics[trim=0 0 0 2.5cm,
  clip,scale=0.31]{figures/adv_sensitivity_non-iid_CIFAR100_resnet18_FedAvg_alpha0.1_clients20_lr0.05_cfgFedAvg_CIFAR100_config_advset0.1-0.2-0.3_epochs200.pdf}}
	\caption{Sensitivity of test accuracy to the attacker ratio on CIFAR-100/ResNet18 under non-IID settings.}
	\label{figaccuracymoreattackerscifar100}
\end{figure*}

\paragraph{Scaling with More Clients.}
\begin{figure*}[!htbp]
	\centering
	\subfloat[Dataset distribution is non-IID with $\alpha=1$.]{\safeincludegraphics[trim=0 0 0 2.5cm,
  ...
  clip,scale=0.31]{figures/num_clients_sensitivity_non-iid_CIFAR100_resnet18_FedAvg_alpha0.1_adv0.2_lr0.05_cfgFedAvg_CIFAR100_config_clientset20-40-60_epochs200.pdf}}
	\caption{Sensitivity of test accuracy to the number of clients on CIFAR-100/ResNet18 under non-IID settings.}
...
\end{figure*}
  \begin{table*}[!htbp]
      ...
      \renewcommand{\arraystretch}{1.0}
      \setlength{\tabcolsep}{4pt}
      \caption{Final test loss on CIFAR-100/ResNet18 under four untargeted model-poisoning attacks at adversary ratio $20\%$, comparing non-IID settings across different
...
\resizebox{\textwidth}{!}{%
  ...
  \pm$0.0006 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0276$\pm$0.0006} &
  ...
  \pm$0.0004 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0290$\pm$0.0004} &
  ...
  \pm$0.0006 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0345$\pm$0.0006} &
  ...
  0.0249$\pm$0.0004 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0271$\pm$0.0003} &
  ...
  \pm$0.0004 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0285$\pm$0.0006} &
  ...
  \pm$0.0007 & 2.0 \\
      & & \cellcolor{mygray}CARAT & \cellcolor{mygray}\textbf{0.0338$\pm$0.0011} &
...
To test whether the method remains effective beyond the default $20$-client setting, we increase the total number of clients to $40$ and $60$ while maintaining an attacker ratio of $20\%$. Fig.~\ref{figaccuracymoreclientscifar100} and Table~\ref{tablosscifar100clientsadv02} show that \texttt{CARAT} continues to converge reliably and retains the best average rank across the scaled CIFAR-100 settings. Competing defenses degrade more sharply, especially under stronger heterogeneity or FangAttack.
...
This section records setup details and benchmark scope that are omitted from the main text for brevity.

\paragraph{Detailed benchmark scope.}
The broader repository configuration includes three suites. The clean-reference suite uses \texttt{NoAttack} and compares \{Mean, FLTrust, MultiKrum, CARAT\}. The untargeted poisoning suite evaluates \{ALIE, MinMax, FangAttack\} against the same defense set. The backdoor suite evaluates \{BadNets, Neurotoxin\} against \{Mean, FLTrust, FLAME, DeepSight, CARAT\}. The committed configs cover both i.i.d.\ and non-i.i.d.\ settings: \texttt{clean\_reference.yaml} and \texttt{main\_untargeted.yaml} use $\alpha \in \{0.1,0.5\}$, while \texttt{backdoor.yaml} uses i.i.d.\ plus $\alpha=0.5$. The untargeted suite sweeps attacker fractions $\{0.2,0.4\}$; the clean-reference and backdoor suites use $0$ and $0.2$, respectively. Each planned suite runs three repeats with effective seeds $42$, $43$, and $44$. The current main text intentionally reports only the completed untargeted slice rather than this full benchmark matrix.

\paragraph{Default CARAT hyperparameters.}
Unless otherwise stated, CARAT uses probe pool size $512$, $8$ hidden tasks, $4$ probe samples per class, probe batch size $128$, probe-radius quantile $0.5$, MAD clipping with $k=2.5$, default attacker prior $\hat{\rho}=0.2$, certificate temperature $\beta=12.0$, rank weight $0.20$, prior weight $0.05$, $4096$ sampled coordinates, $5$ rank subsamples, $60$ optimization steps, and optimizer learning rate $0.5$.

\paragraph{Currently verified run.}
The strongest directly verified completed CARAT run in the repository uses FedAvg, CIFAR100, ResNet-18, an i.i.d.\ partition, 20 clients, 4 attackers, FangAttack, 300 rounds, learning rate $0.05$, local epochs $4$, and \verb|evaluate=false|. The main text uses this run only as preliminary evidence, because the broader clean-reference, backdoor, non-i.i.d., and multi-seed CARAT suites are still being completed.
...
\item Run the planned ablations, prioritizing the number of hidden tasks, the rank and prior weights, and the clipping controls.
