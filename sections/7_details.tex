...
CIFAR-100 is a public dataset. In the reported implementation, CARAT samples a $512$-example server pool from the same public training set used to construct client partitions; those examples are not removed from client local data. This overlap and the unequal CARAT/FLTrust reference budgets are limitations of the current evaluation. A stricter reproduction should additionally test a deterministic, disjoint server split while reporting the resulting reduction in client training data.
