...
\item We provide an auditable CIFAR-100/ResNet18 evaluation against citable robust-aggregation baselines under four untargeted poisoning attacks and non-i.i.d.\ partitions, with matched server-reference budgets for reference-assisted defenses and seed-level reporting.
\end{itemize}
...
The adversary is assumed to know the training protocol, defense family, and public hyperparameters, but not the realized hidden validation tasks sampled by the server in a given round. This is the standard security boundary for CARAT's reference-assisted setting: the labeled reference set and task sampler are server-side audit resources, not public client inputs. Attackers that directly observe the reference examples or optimize against the exact hidden tasks are outside the empirical claim set, although the method still includes clipping, rank penalties, and capped weights to reduce overreliance on the reference signal.
...
The experiments evaluate one deliberately scoped claim: whether hidden-task certificates improve robustness to untargeted model poisoning under heterogeneous client data, when compared against public robust-aggregation baselines under a fixed paper protocol and an explicit result-source policy. The main study uses CIFAR-100/ResNet18, matched trusted-data budgets for CARAT and FLTrust, final attacked accuracy/loss metrics, and average rank across attacks. Appendix~\ref{sec:appendix-experiments} provides protocol details, final-loss results, runtime diagnostics, and reproducibility details.
...
All main attacked runs use $20$ clients with full participation, $200$ global rounds, $4$ local epochs, learning rate $0.05$, and a $20\%$ malicious-client ratio. We evaluate Dirichlet non-i.i.d.\ regimes with $\alpha\in\{1,0.5\}$, where smaller $\alpha$ increases client heterogeneity. Clean no-attack runs are kept separate from attacked robustness tables so that clean utility is not mixed with poisoning resistance. Compatible legacy public-baseline rows use five seeds where available; regenerated FLTrust and CARAT rows use three seeds under the matched reference-assisted protocol. We disclose this source policy in the tables and use average rank to avoid overinterpreting any single seed count. Final test accuracy is the primary metric, final test loss in the appendix is a consistency check, and average rank within each non-i.i.d.\ setting summarizes performance across attacks.
...
Within this reported comparison, CARAT obtains average rank $1.0$ in both heterogeneity settings and is the strongest reported defense in every attack column. Relative to the best non-CARAT row in each column, CARAT improves mean final accuracy by $1.9$--$5.5$ percentage points across the eight attack/heterogeneity cells. The largest margins occur under FangAttack and under the more heterogeneous $\alpha=0.5$ MinMax/MinSum settings, where several non-CARAT defenses lose substantial accuracy. Appendix~\ref{sec:appendix-experiments} reports the corresponding final-loss table.
