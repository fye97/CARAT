...
Formal properties of the normalization step, capped-simplex feasibility, softmin behavior, temporal regularization, and per-round complexity are deferred to Appendix~\ref{sec:appendix-theory}. A compact round-level algorithm summary is provided in Appendix~\ref{sec:appendix-algorithm}.
...
\item Robustly standardize certificate columns to obtain $C^{\mathrm{std}}$, compute rank-space penalties from coordinate subsamples of $\bar{\Delta}_i^r$, and retain only positive standardized rank deviations in $r$.
\item Resolve $\hat{\rho}$, initialize weights from $\alpha^{\mathrm{prev}}$, and optimize
\[
\max_{\alpha \in \mathcal{A}(\hat{\rho})}
\softmin_{\beta}((C^{\mathrm{std}})^\top\alpha)-\lambda_r r^\top\alpha-\lambda_p\|\alpha-\alpha^{\mathrm{prev}}\|_2^2
\]
